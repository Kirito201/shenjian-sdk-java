    ...
    private ResponseParser<NodeInfo> nodeInfoResponseParser = new ResponseParser<NodeInfo>() {
        ...
        public NodeInfo parse(ResponseMessage responseMessage) throws ShenjianException {
            ...
            if (data != null && data instanceof Map) {
                ...
                try {
                    nodeInfo.setTotalNodes(Integer.valueOf(String.valueOf(((Map)data).get("node_all"))));
                    nodeInfo.setRunningNodes(Integer.valueOf(String.valueOf(((Map)data).get("node_running"))));
                    if (((Map)data).get("node_gpu_all") != null) {
                        nodeInfo.setTotalGpuNodes(Integer.valueOf(String.valueOf(((Map) data).get("node_gpu_all"))));
                    }
                    if (((Map)data).get("node_gpu_running") != null) {
                        nodeInfo.setRunningGpuNodes(Integer.valueOf(String.valueOf(((Map) data).get("node_gpu_running"))));
                    }
                    return nodeInfo;
                } catch (NumberFormatException e) {
                    throw new ShenjianException("接口返回异常", e, responseMessage);
