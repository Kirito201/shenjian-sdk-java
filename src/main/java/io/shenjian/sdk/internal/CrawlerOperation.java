...
public class CrawlerOperation extends ShenjianOperation {
            ...
            if (data instanceof Map) {
                ...
                for (Object anArrayList : jsonArray) {
                    ...
                    if (anArrayList instanceof Map) {
                        ...
                        try {
                            crawler.setAppId((Integer) map.get("app_id"));
                            ...
                            crawler.setCreateTime((Integer) map.get("time_create"));
    ...
    private ResponseParser<Float> crawlerSpeedResponseParser = new ResponseParser<Float>() {
        ...
        public Float parse(ResponseMessage responseMessage) throws ShenjianException {
            Object data = responseMessage.getData();
            if (data instanceof Map) {
                Object speed = ((Map) data).get("speed");
                if (speed != null && !(speed instanceof String)) {
                    try {
                        return Float.valueOf(speed.toString());
    ...
    private ResponseParser<CrawlerNodeInfo> crawlerNodeResponseParser = new ResponseParser<CrawlerNodeInfo>() {
        ...
        public CrawlerNodeInfo parse(ResponseMessage responseMessage) throws ShenjianException {
            ...
            if (data instanceof Map) {
                ...
                try {
                    crawlerNodeInfo.setLeftNodes((Integer) map.get("node_left"));
                    crawlerNodeInfo.setRunningNodes((Integer) map.get("node_running"));
        ...
        public CrawlerSource parse(ResponseMessage responseMessage) throws ShenjianException {
            ...
            if (data instanceof Map) {
                ...
                try {
                    ...
                    crawlerSource.setCount((Integer) map.get("count"));
    ...
    public void clearCrawlerData(int appId) throws ShenjianException {
        String path = "crawler/" + appId + "/source/clear";
        Map<String, String> params = getPassportParams();
        doOperation(path, params, emptyResponseParser);
    }

    public void deleteCrawlerData(int appId, int days) throws ShenjianException {
        String path = "crawler/" + appId + "/source/delete";
        Map<String, String> params = new HashMap<String, String>();
        params.putAll(getPassportParams());
    ...
    public void configCrawlerRobotstxt(int appId, boolean isAllow) throws ShenjianException {
        String path = "crawler/" + appId + "/config/robots/agree";
        if(!isAllow){
            path = "crawler/" + appId + "/config/robots/disagree";
        }
        Map<String, String> params = getPassportParams();
        doOperation(path, params, emptyResponseParser);
    }
        ...
        public Webhook parse(ResponseMessage responseMessage) throws ShenjianException {
            ...
            if (data instanceof Map) {
                ...
                try {
                    ...
                    webhook.setEvents(JSON.parseObject(String.valueOf(map.get("events")), String[].class));
                    webhook.setGzip(Boolean.valueOf(String.valueOf(map.get("gzip"))));
                    return webhook;
                } catch (IllegalArgumentException e) {
                    throw new ShenjianException("接口返回异常", e, responseMessage);
                }
        ...
        public AutoPublishStatus parse(ResponseMessage responseMessage) throws ShenjianException {
            ...
            if (data instanceof Map) {
                ...
                try {
                    ...
                    autoPublishStatus.setStopTime((Integer) map.get("time_stop"));
    ...
    public void startAutoPublish(int appId, int[] publishId) throws ShenjianException {
        ...
        params.put("publish_id", JSON.toJSONString(str));
        doOperation(path, params, emptyResponseParser);
    }
